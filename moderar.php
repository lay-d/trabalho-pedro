...
if(isset($_POST['atualiza'])){
    ...
    $sql = "UPDATE lays_1d SET nome='$nome', email='$email', mensagem='$msg' WHERE id=$idatualiza";
...
if(isset($_GET['acao']) && $_GET['acao'] == 'excluir'){
    $id = intval($_GET['id']);
    mysqli_query($conexao, "DELETE FROM lays_1d WHERE id=$id") or die("Erro ao deletar: " . mysqli_error($conexao));
    header("Location: moderar.php");
    exit;
}
...
$recado_editar = null;
if($editar_id){
    $res = mysqli_query($conexao, "SELECT * FROM lays_1d WHERE id=$editar_id");
...
<?php
$seleciona = mysqli_query($conexao, "SELECT * FROM lays_1d ORDER BY id DESC");
if(mysqli_num_rows($seleciona) <= 0){
    echo "<p>Nenhum pedido no mural!</p>";
}else{
    while($res = mysqli_fetch_assoc($seleciona)){
        echo '<ul class="lays_1d">';
